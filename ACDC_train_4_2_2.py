...
def self_train(args, pre_snapshot_path, snapshot_path):
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            topnum = 16
            out_soft_mix = F.softmax(output_mix, dim=1)
            score = torch.max(out_soft_mix, dim=1)[0]  # b, h*p1, w*p2

            # 把每个 patch 的分数做 pooling
            patches_outputs_1 = rearrange(score, 'b (h p1) (w p2)->b (h w) (p1 p2)', p1=4, p2=4)
            patches_outputs_1_mean = torch.mean(patches_outputs_1, dim=-1)  # b, n_patch

            # 取最低置信度的 topnum 个 patch
            patches_mean_1_top_values, patches_mean_1_top_indices = patches_outputs_1.topk(topnum, dim=1, largest=False)
            ...
            # === 关键修改点 ===
            # 这里要从 patch 特征中取，而不是分数
            # 假设你的 patch 特征在变量 patch_feats，形状是 [b, n_patch, d]
            top_patches = torch.gather(patch_feats, 1, patches_mean_1_top_indices.unsqueeze(-1).expand(-1, -1, patch_feats.size(-1)))
            neg_patches = torch.gather(patch_feats, 1, patches_mean_1_remaining_indices.unsqueeze(-1).expand(-1, -1, patch_feats.size(-1)))
