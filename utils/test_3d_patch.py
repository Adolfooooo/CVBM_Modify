...
def var_all_case_LA_argument(model, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, dataset_path=None):
    assert type(dataset_path) is str
    with open(dataset_path + '/test.list', 'r') as f:
        image_list = f.readlines()
    image_list = [dataset_path + '/2018LA_Seg_Training Set/' + item.replace('\n', '') + "/mri_norm2.h5" for item in
...
def var_all_case_LA_argument_parallel(model, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, dataset_path=None):
    assert type(dataset_path) is str
    with open(dataset_path + '/test.list', 'r') as f:
        image_list = f.readlines()
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        results = list(tqdm(executor.map(process_image, image_list, [model]*len(image_list), [stride_xy]*len(image_list),
                                   [stride_z]*len(image_list), [patch_size]*len(image_list), [num_classes]*len(image_list)),
                       total=len(image_list)))
        
    total_dice = sum(results)
    avg_dice = total_dice / len(image_list)
    print(f'Average metric is {avg_dice}')
    return avg_dice
...
def var_all_case_Pancreas_argument(model, num_classes, patch_size=(96, 96, 96), stride_xy=16, stride_z=16, dataset_path=None):
    # dataset_path = '/root/Pancreas'
    assert type(dataset_path) is str
    with open(dataset_path + '/test.list', 'r') as f:
        image_list = f.readlines()
...
def var_all_case_Pancreas_argument_ema(model, ema_model, num_classes, patch_size=(96, 96, 96), stride_xy=16, stride_z=16, dataset_path=None):
    # dataset_path = '/root/Pancreas'
    assert type(dataset_path) is str
    with open(dataset_path + '/test.list', 'r') as f:
        image_list = f.readlines()
...
def test_single_case_argument(model, image, stride_xy, stride_z, patch_size, num_classes=1):
    w, h, d = image.shape
