...
parser.add_argument('--fg2bg_ema', type=float, default=0.99, help='EMA momentum for fg -> bg branch (SKCDF-style)')
parser.add_argument('--snapshot_path', type=str, default='./experiments/11_2/1/results/', help='snapshot path to save model')
...
@torch.no_grad()
def ema_fg_to_bg(model, alpha):
    """
    SKCDF-style knowledge transfer: softly copy fg decoder weights/buffers into
    the bg decoder so bg acts as an EMA student of fg.
    """
    if not hasattr(model, 'decoder_fg') or not hasattr(model, 'decoder_bg'):
        return

    fg_params = dict(model.decoder_fg.named_parameters())
    for name, param in model.decoder_bg.named_parameters():
        if name in fg_params and fg_params[name].shape == param.shape:
            param.data.mul_(alpha).add_((1 - alpha) * fg_params[name].data)

    fg_buffers = dict(model.decoder_fg.named_buffers())
    for name, buf in model.decoder_bg.named_buffers():
        if name in fg_buffers and fg_buffers[name].shape == buf.shape:
            buf.data.mul_(alpha).add_((1 - alpha) * fg_buffers[name].data)
...
def pre_train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            # fg -> bg EMA transfer (SKCDF-style)
            ema_fg_to_bg(model, args.fg2bg_ema)
...
def self_train(args, pre_snapshot_path, self_snapshot_path):
    ...
    for epoch in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            # fg -> bg EMA transfer before forward (both student and EMA model)
            ema_fg_to_bg(model, args.fg2bg_ema)
            ema_fg_to_bg(ema_model, args.fg2bg_ema)
...
if __name__ == "__main__":
    ...
    print("Starting fg->bg EMA training (SKCDF-inspired).")
