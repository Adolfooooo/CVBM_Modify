...
from experiments.cvbm_15_1.t3.prototype_losses import BranchBatchPrototypeLoss
...
def resolve_labelnum():
    ...
    if labelnum >= args.max_samples:
        raise ValueError(
            'semi-supervised training requires labelnum < max_samples; '
            'use la_train_fully_supervision.py for 100% labels'
        )
    return labelnum
...
def self_train(args, pre_snapshot_path, self_snapshot_path):
    ...
    for epoch in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            writer.add_scalar('Self/proto_relation_pairs', proto_stats["proto_relation_pairs"], iter_num)
...
if __name__ == "__main__":
    labelnum = resolve_labelnum()
    pre_snapshot_path = "{}/{}_{}_labeled/pre_train".format(args.snapshot_path, args.exp, labelnum)
    self_snapshot_path = "{}/{}_{}_labeled/self_train".format(args.snapshot_path, args.exp, labelnum)
