...
def parse_3d_tuple(values):
    ...
    if len(parsed) != 3:
        raise argparse.ArgumentTypeError("expected 3 integers, got {}".format(values))
    if any(v <= 0 for v in parsed):
        raise argparse.ArgumentTypeError("all values must be > 0, got {}".format(values))
    return parsed
...
parser.add_argument(
    ...
    type=str,
    default=None,
    help='optional directory containing checkpoints; overrides snapshot_path/exp/labelnum/proto/stage',
)
parser.add_argument(
    '--checkpoint',
    type=str,
...
def test_calculate_metric():
    ...
    if args.test_ema and args.stage_name == 'self_train' and os.path.exists(save_ema_model_path):
        metrics['ema_model'] = evaluate_argument_checkpoint(
            save_ema_model_path,
            'ema_model',
            build_self_train_model,
        )
