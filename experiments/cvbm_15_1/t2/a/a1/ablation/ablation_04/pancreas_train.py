...
parser.add_argument('--exp', type=str, default='CVBM_Pancreas_FgQBgKV_Ablation_ProtoPatch', help='exp_name')
...
def validate_proto_patch_settings(args):
    ...
    for axis_name, feat_dim, patch_dim in zip(("H", "W", "D"), feat_size, args.proto_patch):
        if feat_dim % patch_dim != 0:
            raise ValueError(
                f"proto_patch {args.proto_patch} is incompatible with decoder feature size {feat_size}: "
                f"{axis_name}={feat_dim} is not divisible by {patch_dim}."
            )

    num_proto_patches = (feat_size[0] // args.proto_patch[0]) * (feat_size[1] // args.proto_patch[1]) * (feat_size[2] // args.proto_patch[2])
    ...
    if args.topnum >= num_proto_patches:
        logging.warning(
            ...
            args.topnum, num_proto_patches, args.proto_patch,
        )

    for axis_name, feat_dim, patch_dim in zip(("H", "W", "D"), args.patch_size, args.contrast_patch):
        if feat_dim % patch_dim != 0:
            raise ValueError(
                f"contrast_patch {args.contrast_patch} is incompatible with logits size {args.patch_size}: "
                f"{axis_name}={feat_dim} is not divisible by {patch_dim}."
            )
...
if __name__ == "__main__":
    ...
    print("Starting Pancreas training with SKC3D.")
