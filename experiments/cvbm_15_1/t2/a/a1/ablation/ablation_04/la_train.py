...
# a1 ablation: prototype contrast hyperparameters are the controlled variables.
parser = argparse.ArgumentParser()
parser.add_argument('--root_path', type=str, default='/root/LA', help='Name of Dataset')
parser.add_argument('--exp', type=str, default='CVBM_LA_FgQBgKV_Ablation_ProtoPatch', help='exp_name')
...
parser.add_argument('--patch_size', type=int, nargs=3, default=(112, 112, 80), help='patch_size of loading image')
...
parser.add_argument('--contrast_patch', type=int, nargs=3, default=(8, 8, 8), help='patch size for legacy InfoNCE pooling')
...
parser.add_argument('--snapshot_path', type=str, default='./results/CVBM_15_1_t2_a/a1/ablation_04/1/', help='snapshot path to save model')
...
args.contrast_patch = parse_3d_tuple(args.contrast_patch)
...
def validate_proto_patch_settings(args):
    ...
    for axis_name, feat_dim, patch_dim in zip(("H", "W", "D"), feat_size, args.proto_patch):
        if feat_dim % patch_dim != 0:
            print(
                f"Warning: proto_patch {args.proto_patch} does not evenly divide feature size {feat_size}: "
                f"{axis_name}={feat_dim} is not divisible by {patch_dim}. "
                "avg_pool3d will ignore the trailing border voxels on this axis."
            )
    ...
    if args.topnum >= num_proto_patches:
        logging.warning(
            "topnum=%d is not smaller than pooled patch count=%d under proto_patch=%s. "
            "This does not affect the active prototype branch, but would invalidate the commented legacy InfoNCE branch.",
            args.topnum,
            num_proto_patches,
            args.proto_patch,
        )

    for axis_name, feat_dim, patch_dim in zip(("H", "W", "D"), feat_size, args.contrast_patch):
        if feat_dim % patch_dim != 0:
            raise ValueError(
                f"contrast_patch {args.contrast_patch} is incompatible with logits size {feat_size}: "
                f"{axis_name}={feat_dim} is not divisible by {patch_dim}."
            )
...
def pre_train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        ...
        if iter_num >= pre_max_iterations:
            iterator.close()
            break
    # Short smoke runs may finish before validation has a chance to save a best checkpoint.
    # Keep normal 200-step validation behavior, but provide a compatible fallback for self-training/test loading.
    fallback_best_path = os.path.join(snapshot_path, f'{args.model}_best_model.pth')
    if not os.path.exists(fallback_best_path):
        save_net_opt(model, optimizer, fallback_best_path)
        logging.info("save fallback best model to %s", fallback_best_path)
    ...
    for epoch in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            with torch.no_grad():
                ...
                unoutput_b_fg, unoutput_b, unoutput_b_bg, unoutput_b_sdm, unoutput_b_sdm_bg, _, _ = ema_model(unimg_b, unimg_b_s)
...
if __name__ == "__main__":
    ...
    print("Starting LA prototype patch-size ablation training with SKC3D.")
