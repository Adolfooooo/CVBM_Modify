...
parser.add_argument(
    '--snapshot_path',
    type=str,
    default='./results/CVBM_15_1_t2_a/a1/ablation_06/1/',
    help='snapshot path used by ablation_06/brats19_train.py',
)
...
def evaluate_checkpoint(path, tag, model_builder):
    model = model_builder()
...
def test_calculate_metric_argument():
    ...
    metrics = {'model': evaluate_checkpoint(save_model_path, 'model', model_builder)}
    ...
    if args.test_ema and args.stage_name == 'self_train' and os.path.exists(save_ema_model_path):
        metrics['ema_model'] = evaluate_checkpoint(save_ema_model_path, 'ema model', build_self_train_model)
