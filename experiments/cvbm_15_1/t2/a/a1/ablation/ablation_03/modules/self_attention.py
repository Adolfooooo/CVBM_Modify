...
# a1 ablation_03: replace cross-branch interaction with independent self-attention while keeping the a1 train interface.
...
class BidirectionalCrossAttention(nn.Module):
    """Independent self-attention for foreground and background tokens."""
    ...
    def __init__(self, channels: int, num_heads: int = 4, dropout: float = 0.0) -> None:
        ...
        self.bg_self_attn = nn.MultiheadAttention(
            embed_dim=channels,
            num_heads=num_heads,
            dropout=dropout,
            batch_first=True,
    ...
    def forward(
        ...
        fg_delta, _ = self.fg_self_attn(
            query=fg_query,
            key=fg_context,
            value=fg_context,
            need_weights=False,
        )
        bg_delta, _ = self.bg_self_attn(
            query=bg_query,
            key=bg_context,
            value=bg_context,
            need_weights=False,
...
class LocalCrossAttentionBlock3D(nn.Module):
    """Self-attention inside local non-overlapping 3D windows."""
...
class GlobalCrossAttentionBlock3D(nn.Module):
    """Self-attention over the full bottleneck token sequence."""
