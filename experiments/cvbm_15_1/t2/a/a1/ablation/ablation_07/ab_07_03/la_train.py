...
parser.add_argument('--exp', type=str, default='CVBM_LA_FgStrong_BgStrong_Ablation_07_03', help='exp_name')
...
parser.add_argument('--snapshot_path', type=str, default='./results/CVBM_15_1_t2_a/a1/ablation_07/ab_07_03/1/', help='snapshot path to save model')
...
def pre_train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            outputs_fg, outputs, outputs_bg, *_ = model(volume_batch_strong, volume_batch_strong)
            loss_seg = 0
            loss_seg_dice = 0
            loss_sdf = 0
            ...
            loss_seg_dice += DICE(y_prob2, label_batch_strong[:args.labeled_bs, ...] == 1)
...
def self_train(args, pre_snapshot_path, self_snapshot_path):
    ...
    for epoch in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            with torch.no_grad():
                unoutput_a_fg, unoutput_a, unoutput_a_bg, unoutput_a_sdm, unoutput_a_sdm_bg, _, _ = ema_model(unimg_a_s, unimg_a_s)
                unoutput_b_fg, unoutput_b, unoutput_b_bg, unoutput_b_sdm, unoutput_b_sdm_bg, _, _ = ema_model(unimg_b_s, unimg_b_s)
            ...
            mixl_img_fg = mixl_img_s
            mixl_img_bg = mixl_img_s
            mixu_img_fg = mixu_img_s
            mixu_img_bg = mixu_img_s
            mixl_lab = lab_a_s * img_mask + plab_a * (1 - img_mask)
            mixu_lab = plab_b * img_mask + lab_b_s * (1 - img_mask)
...
if __name__ == "__main__":
    ...
    print("Starting CVBM FG-query-BG Cross-SKC training.")
