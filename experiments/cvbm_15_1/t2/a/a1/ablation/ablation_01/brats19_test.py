...
parser.add_argument('--proto_weight', type=float, default=0.1, help='legacy checkpoint lookup only')
parser.add_argument('--proto_patch', type=ast.literal_eval, default=(8, 8, 8), help='legacy checkpoint lookup only')
...
legacy_proto_dir = "proto_w{}_patch{}".format(args.proto_weight, args.proto_patch)
legacy_snapshot_path = os.path.join(
    args.snapshot_path,
    args.exp,
    'brats19',
...
if args.checkpoint is None:
    default_checkpoint = os.path.join(snapshot_path, '{}_best_model.pth'.format(args.model))
    legacy_checkpoint = os.path.join(legacy_snapshot_path, '{}_best_model.pth'.format(args.model))
    if not os.path.exists(default_checkpoint) and os.path.exists(legacy_checkpoint):
        print("fallback checkpoint path: {}".format(legacy_snapshot_path))
