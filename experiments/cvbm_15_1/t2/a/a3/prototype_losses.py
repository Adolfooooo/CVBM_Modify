...
class BranchBatchPrototypeLoss(nn.Module):
    ...
    def _branch_loss(
        ...
        prototypes, prototype_labels = self._build_sample_prototypes(features, labels, confidence)
        ...
        positive_mask = targets.unsqueeze(1).eq(prototype_labels.unsqueeze(0))
    ...
    def _build_sample_prototypes(
        ...
        if not prototypes:
            return features.new_zeros(0, channels), labels.new_zeros(0, dtype=torch.long)
