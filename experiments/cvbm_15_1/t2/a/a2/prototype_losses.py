...
class BranchBatchPrototypeLoss(nn.Module):
    ...
    def _branch_loss(
        ...
        own_logits = self._prototype_positive_logits(queries, targets, prototypes)
        ...
        for class_idx in range(self.num_classes):
            ...
            other_mask = targets != class_idx
            if not other_mask.any():
                continue

            pos_logits = own_logits[class_mask]
            neg_logits = own_logits[other_mask]
            denominator_logits = torch.cat(
                [
                    pos_logits.unsqueeze(1),
                    neg_logits.unsqueeze(0).expand(pos_logits.numel(), -1),
                ],
                dim=1,
            )
            log_denominator = torch.logsumexp(denominator_logits, dim=1)
            losses.append(-(pos_logits - log_denominator))

        if not losses:
            return features.mean() * 0.0, 0

        return torch.cat(losses).mean(), int(query_idx.numel())

    def _prototype_positive_logits(
        self,
        ...
        return (queries * target_prototypes).sum(dim=1) / self.temperature
