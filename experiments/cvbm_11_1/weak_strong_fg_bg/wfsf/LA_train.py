...
def self_train(args, pre_snapshot_path, self_snapshot_path):
    ...
    for epoch in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            with torch.no_grad():
                unoutput_a_fg,unoutput_a, unoutput_a_bg, unoutput_a_sdm, unsdmput_a_sdm_bg = ema_model(unimg_a, unimg_a)
                unoutput_b_fg,unoutput_b, unoutput_b_bg, unoutput_b_sdm, unsdmput_b_sdm_bg = ema_model(unimg_b, unimg_b)
            ...
            ####
            outputs_l_fg, outputs_l, outputs_l_bg, sdm_outputs_l, sdm_outputs_l_bg = model(mixl_img, mixl_img)
            outputs_u_fg, outputs_u, outputs_u_bg, sdm_outputs_u, sdm_outputs_u_bg = model(mixu_img, mixu_img)
            # output 3x3 connect
            ...
            loss_l_bg = mix_loss(outputs_l_bg, lab_a_s_bg, plab_a_bg, loss_mask, u_weight=args.u_weight)
            loss_u_bg = mix_loss(outputs_u_bg, plab_b_bg, lab_b_s_bg, loss_mask, u_weight=args.u_weight, unlab=True)
