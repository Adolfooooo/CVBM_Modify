...
parser.add_argument('--snapshot_path', type=str, default='./results/CVBM_11_1_aug/1/', help='snapshot path to save model')
...
def load_pretrained_backbone(model, ckpt_path):
    ...
    Args:
        model: Instance of ``CVBMArgumentWithSKC3D`` whose state_dict will be updated.
        ckpt_path: File path that stores the ``{'net': state_dict}`` checkpoint.
    """
...
def self_train(args, pre_snapshot_path, self_snapshot_path):
    # Semantic-knowledge-aware backbone: both weak and strong augmentations are
    # processed by the shared encoder, refined through the SKC module and then
    # decoded by the foreground/background decoders.
    ...
    for epoch in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            with torch.no_grad():
                unoutput_a_fg,unoutput_a, unoutput_a_bg, unoutput_a_sdm, unsdmput_a_sdm_bg = ema_model(unimg_a_s, unimg_a_s)
                unoutput_b_fg,unoutput_b, unoutput_b_bg, unoutput_b_sdm, unsdmput_b_sdm_bg = ema_model(unimg_b_s, unimg_b_s)
                plab_a = get_cut_mask(unoutput_a, nms=1)
                plab_b = get_cut_mask(unoutput_b, nms=1)
                plab_a_s_fg = get_cut_mask(unoutput_a_fg, nms=1)
                plab_b_s_fg = get_cut_mask(unoutput_b_fg, nms=1)
            ...
            ####
            outputs_l_fg, outputs_l, outputs_l_bg, sdm_outputs_l, sdm_outputs_l_bg = model(mixl_img_s, mixl_img_s)
            outputs_u_fg, outputs_u, outputs_u_bg, sdm_outputs_u, sdm_outputs_u_bg = model(mixu_img_s, mixu_img_s)
            # output 3x3 connect
            output_mix_bg_fg = torch.cat([outputs_l, outputs_u], dim=0)

            consistency_weight = get_current_consistency_weight(iter_num // 150)

            loss_l = mix_loss(outputs_l_fg, lab_a, plab_a_s_fg, loss_mask, u_weight=args.u_weight)
            loss_u = mix_loss(outputs_u_fg, plab_b_s_fg, lab_b, loss_mask, u_weight=args.u_weight, unlab=True)
...
if __name__ == "__main__":
    ...
    # shutil.copy('./just_try/LA/LA_train_11_1_aug.py', self_snapshot_path)
    # # -- Pre-Training
