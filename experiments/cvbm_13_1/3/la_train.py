...
from modules import CVBMArgumentWithGlobalCrossSKC3D
...
parser.add_argument('--snapshot_path', type=str, default='./results/CVBM_13_1/3/1/', help='snapshot path to save model')
...
def load_pretrained_backbone(model, ckpt_path):
    ...
    the global-cross / local-self SKC backbone.
...
def self_train(args, pre_snapshot_path, self_snapshot_path):
    # Global-cross / local-self backbone:
    # local bottleneck refinement keeps the original shared self-attention, and
    # explicit fg<->bg interaction is introduced only at the global stage.
    model = CVBMArgumentWithGlobalCrossSKC3D(
        n_channels=1,
        n_classes=num_classes,
        normalization='instancenorm',
        has_dropout=True,
    ).cuda()
    ema_model = CVBMArgumentWithGlobalCrossSKC3D(
        n_channels=1,
        n_classes=num_classes,
        normalization='instancenorm',
        has_dropout=True,
...
if __name__ == "__main__":
    ...
    print("Starting CVBM global-cross training.")
