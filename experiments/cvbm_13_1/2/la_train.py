...
from modules import CVBMArgumentWithLocalCrossSKC3D
...
parser.add_argument('--snapshot_path', type=str, default='./results/CVBM_13_1/2/1/', help='snapshot path to save model')
...
def load_pretrained_backbone(model, ckpt_path):
    ...
    the local-cross / global-self SKC backbone.
...
def self_train(args, pre_snapshot_path, self_snapshot_path):
    # Local-cross / global-self backbone:
    # local bottleneck tokens exchange information explicitly, while the global
    # stage keeps the original shared self-attention refinement.
    model = CVBMArgumentWithLocalCrossSKC3D(
        n_channels=1,
        n_classes=num_classes,
        normalization='instancenorm',
        has_dropout=True,
    ).cuda()
    ema_model = CVBMArgumentWithLocalCrossSKC3D(
        n_channels=1,
        n_classes=num_classes,
        normalization='instancenorm',
        has_dropout=True,
...
if __name__ == "__main__":
    ...
    print("Starting CVBM local-cross training.")
