    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            # origin 
             # out_unl_fg,out_unl, out_unl_bg
            # torch.Size([6, 4, 256, 256]) torch.Size([6, 4, 256, 256]) torch.Size([6, 4, 256, 256])
            out_unl_fg_twice, out_unl_twice, out_unl_bg_twice, _, _ = model(net_input_unl)
            # out_l_fg,out_l, out_l_bg
            # torch.Size([6, 4, 256, 256]) torch.Size([6, 4, 256, 256]) torch.Size([6, 4, 256, 256])
            out_l_fg_twice, out_l_twice, out_l_bg_twice, _, _ = model(net_input_l)

            unl_dice_twice, unl_ce_twice = mix_loss(out_unl_fg_twice, plab_a_fg, lab_a, loss_mask, u_weight=args.u_weight, unlab=True)
            l_dice_twice, l_ce_twice = mix_loss(out_l_fg_twice, lab_b, plab_b_fg, loss_mask, u_weight=args.u_weight)

            unl_dice_bg_twice, unl_ce_bg_twice = onehot_mix_loss(out_unl_bg_twice, plab_a_bg, lab_a_bg, onehot_mask,
                                                        u_weight=args.u_weight, unlab=True)
            l_dice_bg_twice, l_ce_bg_twice = onehot_mix_loss(out_l_bg_twice, lab_b_bg, plab_b_bg, onehot_mask, u_weight=args.u_weight)

            loss_ce_twice = unl_ce_twice + l_ce_twice + unl_ce_bg_twice + l_ce_bg_twice
            loss_dice_twice = unl_dice_twice + l_dice_twice + unl_dice_bg_twice + l_dice_bg_twice
            ### Bidirectional Consistency Loss
            # F.softmax(out_l_fg, dim=1): torch.Size([B, 4, 256, 256])

            loss_consist_l_twice = (consistency_criterion(F.softmax(out_l_fg_twice, dim=1), F.softmax((1 - out_l_bg_twice), dim=1))
                                +consistency_criterion(F.softmax(out_l, dim=1), F.softmax((out_l_fg_twice), dim=1))
                                )
            loss_consist_u_twice = (consistency_criterion(F.softmax(out_unl_fg_twice, dim=1), F.softmax((1 - out_unl_bg_twice), dim=1))
                                +consistency_criterion(F.softmax(out_unl_twice, dim=1), F.softmax((out_unl_fg_twice), dim=1))
                                )
            loss_twice =loss_dice_twice + loss_ce_twice + consistency_weight * (loss_consist_l_twice + loss_consist_u_twice)
