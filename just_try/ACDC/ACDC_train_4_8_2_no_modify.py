...
def get_ACDC_2DLargestCC_soft(segmentation_soft):
    """
    输入:
        segmentation_soft: torch.Tensor, [B, C, H, W]
            模型输出的 softmax 概率图
    输出:
        soft_output: torch.Tensor, [B, H, W]
            每像素 soft 概率（仅保留最大连通区域）
        hard_output: torch.Tensor, [B, H, W]
            对应的硬标签类别索引（仅保留最大连通区域）
    """
    batch_list_soft = []
    batch_list_hard = []

    batch_num, num_classes, H, W = segmentation_soft.shape

    for i in range(batch_num):
        prob_maps = segmentation_soft[i].detach().cpu().numpy()  # [C, H, W]
        pred_label = np.argmax(prob_maps, axis=0)  # [H, W]

        # 初始化输出
        soft_map = np.zeros((H, W), dtype=np.float32)
        hard_map = np.zeros((H, W), dtype=np.int64)

        for c in range(num_classes):
            prob_map = prob_maps[c]  # 当前类别的 soft 概率图
            binary_mask = (pred_label == c).astype(np.uint8)
            ...
            if labels.max() != 0:
                ...
                # soft 标签：在最大连通区域中保留概率值
                soft_map[largestCC] = prob_map[largestCC]

                # hard 标签：标记该区域的类别索引
                hard_map[largestCC] = c

        batch_list_soft.append(soft_map)
        batch_list_hard.append(hard_map)

    # 转为 tensor 并放回 GPU
    soft_output = torch.tensor(np.stack(batch_list_soft), dtype=torch.float32).cuda()
    hard_output = torch.tensor(np.stack(batch_list_hard), dtype=torch.int64).cuda()
...
def get_ACDC_masks(output, nms=0,onehot=False):
    ...
    if nms == 1:
        if onehot:
            probs = get_ACDC_2DLargestCC_onehot(probs, indices)
        else:
...
def get_ACDC_masks_with_confidence(output, nms=0,onehot=False):
    ...
    # if nms == 1:
    soft_output, hard_output = get_ACDC_2DLargestCC_soft(probs)
    return soft_output, hard_output
...
def mix_loss(
    ...
    unlab=False,                    # True: img侧为未标注；False: patch侧为未标注
    unl_conf_mask=None,             # 仅未标注侧传入
    unl_hes_mask=None,
    ...
    patch_region = (1 - mask.detach()).clamp(0, 1)

    unl_region = img_region if unlab else patch_region
    lab_region = patch_region if unlab else img_region
    unl_target = img_l if unlab else patch_l
    lab_target = patch_l if unlab else img_l

    # ---------- 标注侧（整块监督） ----------
    if lab_region.sum() > 0:
        loss_dice_lab = dice_loss(
            output_soft, lab_target.unsqueeze(1), lab_region.unsqueeze(1)
        )
        ce_pix = CE(output, lab_target)
        loss_ce_lab = (ce_pix * lab_region).sum() / (lab_region.sum() + _EPS)
    else:
        loss_dice_lab = output.sum() * 0.0
        loss_ce_lab   = output.sum() * 0.0

    loss_dice_lab *= l_weight
    loss_ce_lab   *= l_weight

    # ---------- 未标注侧（仅 conf/hes；rest 剔除） ----------
    if unl_region.sum() > 0:
        conf_region = ((unl_region * unl_conf_mask.detach()).clamp(0, 1)
                       if unl_conf_mask is not None else unl_region.new_zeros(unl_region.shape))
        hes_region  = ((unl_region * unl_hes_mask.detach()).clamp(0, 1)
                       if unl_hes_mask  is not None else unl_region.new_zeros(unl_region.shape))

        has_conf = conf_region.sum() > 0
        has_hes  = hes_region.sum()  > 0

        # Dice：对子区分别计算后按 (w_conf, w_hes) 归一融合
        dice_conf = (dice_loss(output_soft, unl_target.unsqueeze(1), conf_region.unsqueeze(1))
                     if has_conf else output.sum() * 0.0)
        dice_hes  = (dice_loss(output_soft, unl_target.unsqueeze(1), hes_region.unsqueeze(1))
                     if has_hes  else output.sum() * 0.0)
        loss_dice_unl = _avg2(dice_conf, w_conf if has_conf else 0.0,
                              dice_hes,  w_hes  if has_hes  else 0.0)

        # CE：对子区分别取像素均值，再按 (w_conf, w_hes) 归一融合
        ce_pix = CE(output, unl_target)
        ce_conf = ((ce_pix * conf_region).sum() / (conf_region.sum() + _EPS)
                   if has_conf else output.sum() * 0.0)
        ce_hes  = ((ce_pix * hes_region ).sum() / (hes_region.sum()  + _EPS)
                   if has_hes  else output.sum() * 0.0)
        loss_ce_unl = _avg2(ce_conf, w_conf if has_conf else 0.0,
                            ce_hes,  w_hes  if has_hes  else 0.0)
    else:
        loss_dice_unl = output.sum() * 0.0
        loss_ce_unl   = output.sum() * 0.0

    loss_dice_unl *= u_weight
    loss_ce_unl   *= u_weight
...
def onehot_mix_loss(
    output,
    img_l_onehot, patch_l_onehot,   # one-hot 标签
    mask,
    l_weight=1.0, u_weight=0.5,
    unlab=False,
    unl_conf_mask=None,
    ...
    unl_region   = img_region if unlab else patch_region
    lab_region   = patch_region if unlab else img_region
    unl_targetOH = img_l_onehot if unlab else patch_l_onehot
    lab_targetOH = patch_l_onehot if unlab else img_l_onehot

    # 标注侧
    if lab_region.sum() > 0:
        loss_dice_lab = onehot_dice_loss(output_soft, lab_targetOH, lab_region)
        loss_ce_lab   = onehot_ce_loss (output_soft, lab_targetOH, lab_region)
    else:
        loss_dice_lab = output.sum() * 0.0
        loss_ce_lab   = output.sum() * 0.0
    loss_dice_lab *= l_weight
    ...
    if unl_region.sum() > 0:
        ...
        dice_conf = (onehot_dice_loss(output_soft, unl_targetOH, conf_region)
                     if has_conf else output.sum() * 0.0)
        dice_hes  = (onehot_dice_loss(output_soft, unl_targetOH, hes_region)
                     if has_hes  else output.sum() * 0.0)
        loss_dice_unl = _avg2(dice_conf, w_conf if has_conf else 0.0,
                              dice_hes,  w_hes  if has_hes  else 0.0)

        ce_conf = (onehot_ce_loss(output_soft, unl_targetOH, conf_region)
                   if has_conf else output.sum() * 0.0)
        ce_hes  = (onehot_ce_loss(output_soft, unl_targetOH, hes_region)
                   if has_hes  else output.sum() * 0.0)
        loss_ce_unl = _avg2(ce_conf, w_conf if has_conf else 0.0,
                            ce_hes,  w_hes  if has_hes  else 0.0)
    else:
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            unl_dice, unl_ce = mix_loss(out_mix_a_fg, plab_a_fg, lab_a, loss_mask, u_weight=args.u_weight, unlab=True, 
                                            unl_conf_mask=mask_plab_a_fg_confident, unl_hes_mask=mask_plab_a_fg_hesitant)
