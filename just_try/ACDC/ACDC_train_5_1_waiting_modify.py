...
from utils.bhc_utils import (
    ProjectionHead, build_neg_mask_batch, select_bhc_patches,
...
def new_contrastive(args, model, output:list):
    ...
    # BHC 超参（按你的2D先跑起步值）
    tau_notfg = 0.70      # 弱视图（前景分支）的“非前景”阈值
    ...
    weak_is_multiclass = None   # None=自动判断；也可手动 True/False
    strong_is_multiclass = None
    ...
    neg_mask = build_neg_mask_batch(
        weak_list   = [out_unl_fg, out_l_fg],
        strong_list = [out_unl_bg, out_l_bg],
        tau_notfg=tau_notfg, tau_bg=tau_bg,
        bg_index=bg_index, fg_indices=fg_indices,
        weak_is_multiclass=weak_is_multiclass,
        strong_is_multiclass=strong_is_multiclass
    )  # [B,1,H,W]
