    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            with torch.no_grad():
                ...
                plab_a_bg_s = get_ACDC_masks(pre_a_bg_s, nms=1)
                plab_b_bg_s = get_ACDC_masks(pre_b_bg_s, nms=1)
                plab_a_bg_s_onehot = get_ACDC_masks(pre_a_bg_s, nms=1,onehot=True)
                plab_b_bg_s_onehot = get_ACDC_masks(pre_b_bg_s, nms=1,onehot=True)
...
if __name__ == "__main__":
    ...
    shutil.copy('./just_try/ACDC/ACDC_train_4_2.py', self_snapshot_path)
