...
parser.add_argument('--snapshot_path', type=str, default='./results/ACDC_train_4_6_3_mix_conv2d/1', help='snapshot_path')
...
def self_train(args, pre_snapshot_path, snapshot_path):
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            out_fg = torch.concat([out_unl_fg, out_l_fg], dim=0)
            out_bg = torch.concat([out_unl_bg, out_l_bg], dim=0)
            output_mix = model.final_seg(torch.cat([out_fg, out_bg], dim=1))
...
if __name__ == "__main__":
    ...
    shutil.copy('./just_try/ACDC/ACDC_train_4_6_3_mix_conv2d.py', self_snapshot_path)
