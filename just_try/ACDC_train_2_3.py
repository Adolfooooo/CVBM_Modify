...
from networks.ABD import ABD_R_BCP
...
def self_train(args, pre_snapshot_path, snapshot_path):
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            #                     )
            # loss_1 =loss_dice + loss_ce + consistency_weight * (loss_consist_l + loss_consist_u)

            # ABD-R New Training Sample
            # out_max_1.shape, out_max_2.shape, net_input.shape, out_1.shape, out_2.shape
            # torch.Size([12, 256, 256]) torch.Size([12, 256, 256]) torch.Size([12, 1, 256, 256]) torch.Size([12, 4, 256, 256]) torch.Size([12, 4, 256, 256])
            net_input = torch.cat([net_input_unl, net_input_l], dim=0)
            out_1, out_2 = torch.cat([out_unl_fg, out_l_fg], dim=0), torch.cat([out_unl_bg, out_l_bg], dim=0)

            out_soft_1, out_soft_2 = torch.softmax(out_1, dim=1), torch.softmax(out_2, dim=1)

            out_max_1, out_max_2 = \
                torch.max(out_soft_1.detach(), dim=1)[0], torch.max(out_soft_2.detach(), dim=1)[0]

            out_pseudo_1, out_pseudo_2 = \
                torch.argmax(out_soft_1.detach(), dim=1, keepdim=False), torch.argmax(out_soft_2.detach(), dim=1, keepdim=False)
            
            def pseudo_reverse(pseudo, num_classes):
                pseudo_one_hot = ~(F.one_hot(pseudo, num_classes=num_classes).bool()).int()
                pseudo_one_new = torch.argmax(pseudo_one_hot.detach(), dim=3)
                return pseudo_one_new

            out_pseudo_1_new, out_pseudo_2_new = pseudo_reverse(out_pseudo_1, 4), pseudo_reverse(out_pseudo_2, 4)
            ...
            #                     +consistency_criterion(F.softmax(out_unl, dim=1), F.softmax((out_unl_fg), dim=1))
            #                     )
            pseudo_supervision1 = dice_loss(out_soft_1, out_pseudo_2_new.unsqueeze(1))  
            pseudo_supervision2 = dice_loss(out_soft_2, out_pseudo_1_new.unsqueeze(1))  
            # image_patch_last:torch.Size([24, 256, 256])
            image_patch_last = ABD_R_BCP(out_max_1, out_max_2, net_input, net_input, out_1, out_2, args)

            out_fg_abd, out_abd, out_bg_abd, _, _ = model(image_patch_last.unsqueeze(1))

            # print(out_bg_abd.shape, out_fg_abd.shape, out_abd.shape)
            # import sys
            # sys.exit()
            # Model1 & Model2 Second Step Cross Pseudo Supervision
            # loss_consist_abd = (consistency_criterion(F.softmax(out_fg_abd, dim=1), 1 - F.softmax((out_bg_abd), dim=1))
            #                     +consistency_criterion(F.softmax(out_abd, dim=1), F.softmax((out_fg_abd), dim=1))
            #                     )

            image_output_soft_1, image_output_soft_2 = torch.softmax(out_fg_abd, dim=1), torch.softmax(out_bg_abd, dim=1)
            pseudo_image_output_1, pseudo_image_output_2 = \
                torch.argmax(image_output_soft_1.detach(), dim=1, keepdim=False), torch.argmax(image_output_soft_2.detach(), dim=1, keepdim=False)
            pseudo_supervision3 = dice_loss(image_output_soft_1, pseudo_reverse(pseudo_image_output_2, 4).unsqueeze(1))
