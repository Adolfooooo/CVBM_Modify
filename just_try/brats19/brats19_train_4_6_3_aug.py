...
def self_train(args, pre_snapshot_path, self_snapshot_path):
    model = net_factory(net_type=args.model, in_chns=1, class_num=args.num_classes, mode="train")
    ema_model = net_factory(net_type=args.model, in_chns=1, class_num=args.num_classes, mode="train")
    for param in ema_model.parameters():
        param.detach_()  # ema_model set
    db_train = BRATSDataset(base_dir=args.root_path,
                       split='train',
                       transform=transforms.Compose([
                            WeakStrongAugment3d(args.patch_size, flag_rot=False)
                       ]))
    ...
    for epoch in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            pos_patches, neg_patches = select_patches_for_contrast_3d(output_mix_bg_fg, topnum=20, patch_size=(8, 8, 24), choose_largest=False)
            ...
            if iter_num % 400 == 0:
                ...
                dice_sample = test_3d_patch.var_all_case_BRATS19_argument(model, num_classes=args.num_classes, patch_size=args.patch_size,
                                                            stride_xy=18, stride_z=4, dataset_path=args.root_path)
                ema_dice_sample = test_3d_patch.var_all_case_BRATS19_argument(ema_model, num_classes=args.num_classes, patch_size=args.patch_size,
                                                            stride_xy=18, stride_z=4, dataset_path=args.root_path)
                if dice_sample > best_dice:
...
if __name__ == "__main__":
    ...
    shutil.copy('./just_try/brats19/brats19_train_4_6_3_aug.py', self_snapshot_path)
    # # -- Pre-Training
