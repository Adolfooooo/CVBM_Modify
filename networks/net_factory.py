...
def net_factory(net_type="unet", in_chns=1, class_num=4, mode = "train"):
    ...
    elif net_type == "CVBM2d_Argument":
        net = CVBM2d_Argument(in_chns=in_chns, class_num=class_num).cuda()
    elif net_type == "CVBM_Argument" and mode == "train":
        net = CVBM_Argument(n_channels=in_chns, n_classes=class_num, normalization='instancenorm', has_dropout=True).cuda()
    elif net_type == "CVBM_Argument" and mode == "test":
        net = CVBM_Argument(n_channels=in_chns, n_classes=class_num, normalization='instancenorm', has_dropout=False).cuda()
